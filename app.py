...
# Python의 날짜/시간 객체를 JSON으로 변환하는 클래스
...
def init_db():
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                "DROP TABLE IF EXISTS PostReadStatus, Posts, Comments, Tasks, Projects, Users CASCADE;"
            )

            # 테이블 생성 (Users, Projects, Tasks, Comments, Posts, PostReadStatus)
            # ... (이전과 동일한 CREATE TABLE 구문) ...
...
def get_all_data():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM Projects ORDER BY status, priority, deadline ASC"
            )
...
def create_user():
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                "INSERT INTO Users(name) VALUES (%s) ON CONFLICT (name) DO NOTHING RETURNING *",
                (name,),
            )
...
def create_project():
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            if tasks:
                rows = [
                    (project_id, t["content"], _parse_date(t.get("deadline")))
                    for t in tasks
                    if t.get("content")
                ]
                if rows:
                    psycopg2.extras.execute_batch(
                        cur,
                        "INSERT INTO Tasks (project_id, content, deadline) VALUES (%s,%s,%s)",
                        rows,
                    )
            cur.execute("SELECT * FROM Projects WHERE id = %s", (project_id,))
            new_project = cur.fetchone()
            cur.execute(
                "SELECT * FROM Tasks WHERE project_id = %s ORDER BY id ASC",
                (project_id,),
            )
...
def update_project(project_id):
    data = request.get_json() or {}
    fields, params = [], []
    for key in ("name", "priority", "progress", "deadline", "category"):
        if key in data:
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                f"UPDATE Projects SET {', '.join(fields)} WHERE id=%s", tuple(params)
            )
            cur.execute("SELECT * FROM Projects WHERE id = %s", (project_id,))
            ...
            if updated_project:
                ...
                tasks = cur.fetchall()
                cur.execute(
                    "SELECT * FROM Comments WHERE project_id = %s ORDER BY created_at ASC",
                    (project_id,),
                )
                comments = cur.fetchall()
...
def create_task(project_id):
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                "INSERT INTO Tasks (project_id, content) VALUES (%s,%s) RETURNING *",
                (project_id, ""),
            )
...
def update_task(task_id):
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                f"UPDATE Tasks SET {', '.join(fields)} WHERE id=%s", tuple(params)
            )
...
def create_comment(project_id):
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                "INSERT INTO Comments (project_id, author_name, content) VALUES (%s,%s,%s) RETURNING *",
...
def update_comment(comment_id):
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                "UPDATE Comments SET content=%s WHERE id=%s RETURNING *",
                (content, comment_id),
            )
...
def create_post():
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            cur.execute(
                "SELECT p.*, u.name as author_name FROM Posts p JOIN Users u ON p.user_id = u.id WHERE p.id = %s",
                (post_id,),
            )
