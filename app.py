    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                JOIN Users u ON s.user_id = u.id ORDER BY s.schedule_date ASC
            """
            )
            ...
            return jsonify(
                {
                    ...
                    "users": users,
                    "posts": posts,
                    "schedules": schedules,  # 응답에 schedules 추가
...
def create_user():
    ...
    name = data.get("name")
    position = data.get("position")  # position 값 받기
    if not name:
        return jsonify({"status": "error", "message": "name is required"}), 400
    conn = get_db_connection()
    try:
        with conn, conn.cursor() as cur:
            # INSERT 구문에 position 추가
            cur.execute(
                "INSERT INTO Users(name, position) VALUES (%s, %s) ON CONFLICT (name) DO NOTHING RETURNING *",
                (name, position),
            )
...
def update_user(user_id):
    position = (request.get_json() or {}).get("position")
    conn = get_db_connection()
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                "UPDATE Users SET position=%s WHERE id=%s RETURNING *",
                (position, user_id),
            )
...
def create_project():
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            if tasks:
                rows = [
                    (project_id, t["content"], _parse_date(t.get("deadline")), i + 1)
                ...
                if rows:
                    psycopg2.extras.execute_batch(
                        cur,
                        "INSERT INTO Tasks (project_id, content, deadline, task_order) VALUES (%s,%s,%s,%s)",
                        rows,
                    )
            cur.execute("SELECT * FROM Projects WHERE id = %s", (project_id,))
            new_project = cur.fetchone()
            cur.execute(
                "SELECT * FROM Tasks WHERE project_id = %s ORDER BY task_order ASC",
                (project_id,),
            )
...
def create_task(project_id):
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                "SELECT COALESCE(MAX(task_order), 0) + 1 AS new_order FROM Tasks WHERE project_id = %s",
                (project_id,),
            )
            new_order = cur.fetchone()["new_order"]
            cur.execute(
                "INSERT INTO Tasks (project_id, content, task_order) VALUES (%s,%s,%s) RETURNING *",
...
def reorder_tasks():
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            psycopg2.extras.execute_batch(
                cur, "UPDATE Tasks SET task_order=%s WHERE id=%s", update_data
            )
        return jsonify({"status": "success"})
    finally:
        conn.close()
...
def create_schedule():
    data = request.get_json() or {}
    required = ["user_id", "content", "schedule_date"]
    if any(k not in data for k in required):
        return (
            jsonify({"status": "error", "message": "필수 정보가 누락되었습니다."}),
            400,
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                "INSERT INTO Schedules (user_id, content, schedule_date, schedule_type) VALUES (%s, %s, %s, %s) RETURNING id",
                (data["user_id"], data["content"], data["schedule_date"], data["schedule_type"]),
            )
            schedule_id = cur.fetchone()["id"]
            cur.execute(
                """
                SELECT s.*, u.name as user_name FROM Schedules s 
                JOIN Users u ON s.user_id = u.id WHERE s.id = %s
            """,
                (schedule_id,),
            )
